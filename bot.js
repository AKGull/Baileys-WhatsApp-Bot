...
async function start() {
  ...
  bot.ev.on("messages.upsert", async ({ type, messages }) => {
    ...
    const lid = msg.key.participant ?? jid;
    ...
    if (msg) {
      await storeMessage({ jid, lid, key, text });
    }
  ...
  bot.ev.on("messages.update", async (updates) => {
    for (const update of updates) {
      ...
      const editedText = update.update?.message?.conversation || null;
      ...
      if (editedText) {
        recordEdit(bot, jid, msgId, editedText);
      }
